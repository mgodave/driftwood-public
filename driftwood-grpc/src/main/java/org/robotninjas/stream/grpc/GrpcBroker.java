/**
 * Copyright [2023] David J. Rusek <dave.rusek@gmail.com>
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.robotninjas.stream.grpc;

import org.robotninjas.stream.StreamFactory;
import org.robotninjas.stream.admin.AdminRepository;
import org.robotninjas.stream.broker.AckTracker;
import org.robotninjas.stream.broker.AckTrackerFactory;
import org.robotninjas.stream.broker.BrokerSupport;
import org.robotninjas.stream.broker.OffsetStore;
import org.robotninjas.stream.cluster.ResourceLocator;
import org.robotninjas.stream.proto.Acknowledge;
import org.robotninjas.stream.proto.Publish;
import org.robotninjas.stream.proto.ReactorBrokerGrpc;
import org.robotninjas.stream.proto.Subscribe;
import org.robotninjas.stream.security.Security;
import reactor.core.publisher.Flux;
import reactor.core.publisher.Mono;

public class GrpcBroker {
  public static ReactorBrokerGrpc.BrokerImplBase server(
    StreamFactory streamFactory,
    ResourceLocator resourceLocator,
    AdminRepository adminRepository,
    Security security) {
    return new GrpcBrokerFacade(
      new BrokerSupport(
        streamFactory,
        resourceLocator,
        adminRepository,
        OffsetStore.NullStore,
        AckTrackerFactory.Null,
        security
      ));
  }

  private static class GrpcBrokerFacade extends ReactorBrokerGrpc.BrokerImplBase {
    private final BrokerSupport support;

    private GrpcBrokerFacade(BrokerSupport support) {
      this.support = support;
    }

    @Override
    public Flux<Subscribe.Response> subscribe(Mono<Subscribe.Request> request) {
      var authToken = ContextKeys.AuthTokenContextKey.get();
      return support.subscribe(request, new GrpcCallContext(authToken))
        .onErrorMap(ErrorMapper::toStatus);
    }

    @Override
    public Flux<Publish.Response> publish(Flux<Publish.Request> request) {
      var authToken = ContextKeys.AuthTokenContextKey.get();
      return support.publish(request, new GrpcCallContext(authToken))
        .onErrorMap(ErrorMapper::toStatus);
    }

    @Override
    public Flux<Acknowledge.Response> acknowledge(Flux<Acknowledge.Request> request) {
      var authToken = ContextKeys.AuthTokenContextKey.get();
      return support.acknowledge(request, new GrpcCallContext(authToken))
        .onErrorMap(ErrorMapper::toStatus);
    }
  }
}
